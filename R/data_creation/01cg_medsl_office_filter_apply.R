## Apply the OFFICE filter (exclude_other_offices in 01a) to ALL MEDSL-sourced House and Senate rows in the panel (2026-09-20).
## MEDSL's 2016 files contain straight-party/straight-ticket lines (AL, SC, IN, KY), Arkansas "unopposed candidates" ballot rows and DC delegate/shadow
## offices; 01a counted them as House/Senate votes (AL/SC/AR/IN 2016 totals were 1.45-1.67x the presidential total). Same structure as 01ce:
## gate (the panel's MEDSL rows must equal the previous final targets in medsl_final_targets.rds), then replace/remove rows, refresh the MEDSL source files.
## Outputs: R/output/medsl_final_targets_v2.rds, medsl_office_filter_changes.csv, medsl_office_filter_removed_rows.csv, updated panel.
...
tf <- file.path(OUTPUT_DIR, "medsl_final_targets_v2.rds")
if (file.exists(tf) && !identical(Sys.getenv("RECOMPUTE"), "1")) { message("reusing saved final targets"); all_f <- readRDS(tf)
  ...
  save_step(all_f, "medsl_final_targets_v2")
}
...
oldf <- readRDS(file.path(OUTPUT_DIR, "medsl_final_targets.rds"))          # the previous final targets (01ce)
...
write.csv(summ, file.path(OUTPUT_DIR, "medsl_office_filter_changes.csv"), row.names = FALSE)
write.csv(removed %>% transmute(year, cty_fips, sample, state_fips = cty_fips %/% 1000, demovote = demovote.old, repuvote = repuvote.old, totalvote = totalvote.old),
          file.path(OUTPUT_DIR, "medsl_office_filter_removed_rows.csv"), row.names = FALSE)
