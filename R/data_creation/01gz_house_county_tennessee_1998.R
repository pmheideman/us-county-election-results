## Tennessee U.S. House, county level, 1998, from the Secretary of State's archived county report (image-only PDF, OCR'd and checked: 01gy_tennessee_1998_parse.py ->
## R/data/county_house_files/tennessee/tennessee_house_county_1998.csv; 95 counties, every district's candidate columns tie to its printed DISTRICT TOTAL). A county split between districts
## (Bradley, Knox, Davidson, Robertson, Shelby) appears in each and is summed. Party: D -> DEM, R -> REP, everything else OTHER (independents).
## Outputs: he_tnsos_1998 long and elect_he_cty_tnsos_1998 (folded in by 01ha_tennessee_1998_apply.R).
source(file.path("R", "00_setup.R")); source(file.path("R", "long_helpers.R"))
library(readr)
d <- read_csv(file.path(PROJECT_ROOT, "R/data/county_house_files/tennessee/tennessee_house_county_1998.csv"), show_col_types = FALSE, col_types = cols(.default = "c")) %>%
...
long <- finalize_long(raw, "tnsos_1998"); save_long(long, "he_tnsos_1998")
...
f <- file.path(OUTPUT_DIR, "elect_he_cty_tnsos_1998.rds"); saveRDS(shares, f); r <- check_long_vs_source(long, f); stopifnot(all(r$pass))
