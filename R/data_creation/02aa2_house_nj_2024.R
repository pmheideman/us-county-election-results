## New Jersey 2024 U.S. House by county, from the Division of Elections' official results (Official List, 01/18/2023):
## https://www.nj.gov/state/elections/assets/pdf/election-results/2024/2024-official-general-results-us-house.pdf (saved in R/data/raw_house_county_open_states/new_jersey_doe/; text layer).
...
## Same parser as 02aa (2022), with '!?/()' allowed in ballot-line slogans ('ALL OF US!' in District 10). MEDSL's 2024 New Jersey House rows lack Sussex County, double Bergen and carry blank-party / nonpartisan rows; this replaces them. Checks: county rows add up to each candidate's printed Total.
## Outputs: R/output/long/he_nj_2024.rds, R/output/elect_he_cty_nj_2024.rds (NEW files).
source(file.path("R", "00_setup.R")); source(file.path("R", "long_helpers.R")); library(readr)
PDF <- file.path(PROJECT_ROOT, "R/data/raw_house_county_open_states/new_jersey_doe/2024-official-general-results-us-house.pdf")
...
raw <- raw %>% transmute(year = 2024L, county_fips, district, candidate = pretty_name(candidate), party_group = case_when(party == "DEMOCRATIC" ~ "DEM", party == "REPUBLICAN" ~ "REP", TRUE ~ "OTHER"), party = pretty_party(party), votes)   # party_group BEFORE the label
long <- finalize_long(raw, "nj_2024"); save_long(long, "he_nj_2024")
sh <- derive_shares(long) %>% transmute(state = "NEW JERSEY", year, cty_fips, sample, demovote, repuvote, totalvote); saveRDS(sh, file.path(OUTPUT_DIR, "elect_he_cty_nj_2024.rds"))
print(check_long_vs_source(long, file.path(OUTPUT_DIR, "elect_he_cty_nj_2024.rds")) %>% select(source, keys_source, matched, pass))
...
cat("House 2024 / presidential 2020 total: min", round(min(r$ratio), 2), "median", round(median(r$ratio), 2), "max", round(max(r$ratio), 2), "\n")
old <- pe %>% filter(sample == "HE", year == 2024, cty_fips %/% 1000 == 34) %>% select(cty_fips, o_tot = totalvote); cat("MEDSL panel NJ 2024 rows:", nrow(old), "| their total votes", sum(old$o_tot), "vs official", sum(sh$totalvote), "\n")
