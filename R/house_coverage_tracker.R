...
MANUAL_NOTES <- tribble(
  ...
  "CONNECTICUT",     NA,  "Bot-blocked: portaldir.ct.gov (WAF \"Request Rejected\"). Needs a manual browser visit.",
  ...
  "NEW YORK",         NA,  "DONE 2000,2004,2006,2014 (full) + 2012 (partial, 58/62) via OpenElections (github.com/openelections/openelections-data-ny) -- see 01o_house_county_new_york.R. NY has a real electoral-fusion system (a candidate can appear on multiple parties' ballot lines, e.g. Democratic + Working Families) -- handled by collapsing to (county,district,candidate) and summing ALL of a candidate's lines before classifying DEM/REP, rather than trusting any single row's party field. Hit and fixed two pseudo-candidate rows that silently doubled totalvote if not excluded ('Total' and, in the 2012 precinct file only, 'Ballots Cast'/'Over Votes'/'Under Votes'). 2012 has a real, uncorrectable gap: Chautauqua/Chemung/Montgomery/Onondaga have no U.S. House rows at all that year. 2002 EXCLUDED (file exists but has no U.S. House race at all that year -- OpenElections archive gap). 2008/2010 EXCLUDED (repo only has 1-2 counties' worth of precinct files, not statewide).",
