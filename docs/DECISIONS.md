...
"confirm" in section 9 are checked; if the Auburn spreadsheet terms do not permit redistribution, the Alabama 1990-2012 rows
would be dropped or rebuilt from another source before release. The copyright holder line in the license files is a placeholder.
...
| Auburn University Alabama election archive (spreadsheet supplied by the user) | Alabama House 1980-2012 | **Terms unknown; confirm before release** |
...
- **Source tokens and SOURCES.csv (2026-09-21):** the long file's `source` column is now `<state>_<origin>_<year>` (e.g. `in_sos_report_1998`, `tx_sos_web_1996`, `nc_medsl_2018`), with origins medsl / openelections / sos_report / sos_file / sos_web / academic_xls. `release/.../SOURCES.csv` has one row per token (publisher, document, locator, format, how obtained and transcribed, license, license status, related corrections-log entries). Built from `R/data/source_registry.csv` (hand-maintained; the release script stops if a source has no registry entry) by `R/source_tokens.R`. **No license has been verified**: MEDSL is believed CC0, OpenElections and the Auburn Alabama workbook need checking, state sources state no terms. Also: when several sources hold an identical row, the per-year official build now wins over the state-wide OpenElections file in `build_provenance.R`.
