# Data dictionary: v1.0.0 (U.S. House, President and U.S. Senate)

Cite as: Heideman, Paul (2026). *U.S. county-level election results: President, U.S. House and U.S. Senate, 1990-2024* (v1.0.0) [Data set]. Zenodo. https://doi.org/10.5281/zenodo.22976553
...
All offices share the same tables; the `office` column is `house`, `president` or `senate`.

Files in this release: `us_county_results_long.csv` (and `us_county_results_long.parquet`), `us_county_results_summary.csv`, `us_county_results_gaps.csv`,
`us_county_results_no_ballot.csv`, `SOURCES.csv`, `data_corrections_log.csv`, `DATA_DICTIONARY.md` (this file), `CHANGELOG.md`, `VERSION` and `SHA256SUMS.txt`.
All CSVs are UTF-8 with a header row; empty cells are empty strings (no `NA`). Read FIPS codes and `district` as text so leading zeros are kept.

## us_county_results_long.parquet
The same rows and columns as `us_county_results_long.csv`, with `year` and `votes` stored as integers and every other column as text
(so `state_fips`, `county_fips` and `district` keep their leading zeros). Read it with `arrow::read_parquet()` or `nanoparquet::read_parquet()` in R,
or `pandas.read_parquet()` / `polars.read_parquet()` in Python.
